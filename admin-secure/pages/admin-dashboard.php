        ...
        if (response.success) {
            ...
            container.innerHTML = `
                ...
                <div class="quick-stat">
                    ...
                    <div class="quick-stat-info">
                        <span class="value" id="aiCallsToday">—</span>
...
// Load AI stats for dashboard quick-stat widget
function loadAiQuickStat() {
    const el = document.getElementById('aiCallsToday');
    if (!el) return;
    fetch(document.getElementById('baseUrl').value + 'admin-secure/ajax/settings.php?action=get_ai_stats')
        .then(r => r.json()).then(d => {
            if (d.success) el.textContent = formatNumber(d.calls_today);
        }).catch(() => {});
}

// Initialize on load
document.addEventListener('DOMContentLoaded', () => { initDashboard(); loadAiQuickStat(); });
</script>
