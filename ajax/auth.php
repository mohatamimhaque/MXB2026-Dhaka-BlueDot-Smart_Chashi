...
if ($action === 'send_code') {
    ...
    if (!$hasUppercase) {
        $response['message'] = 'Password must contain at least one uppercase letter (A-Z)';
        echo json_encode($response);
        exit;
    }
    
    if (!$hasLowercase) {
        $response['message'] = 'Password must contain at least one lowercase letter (a-z)';
        echo json_encode($response);
        exit;
    }
    
    if (!$hasNumber) {
        $response['message'] = 'Password must contain at least one number (0-9)';
        echo json_encode($response);
        exit;
    }
    
    if (!$hasSpecial) {
        $response['message'] = 'Password must contain at least one special character (!@#$%^&* etc.)';
        echo json_encode($response);
        exit;
    }
    ...
    $db->query("INSERT INTO users (email, phone, password_hash, first_name, last_name, role, is_active, is_verified, profile_img_url) 
                VALUES (?, ?, ?, ?, ?, ?, 1, 0, '')")
       ->bind(1, $email)
       ->bind(2, $phone)
       ->bind(3, $passwordHash)
...
if ($action === 'verify_code') {
    ...
    // Check if this is a registration flow (user has no profile photo) or login flow
    if (empty($user['profile_img_url']) || $user['profile_img_url'] === 'uploads/profiles/default-avatar.jpg') {
        // Registration flow - redirect to profile completion
        $response['redirect'] = 'register?step=profile';
        // Keep these for profile step
        // Don't clean up pending sessions yet
    } else {
        // Login flow - user already has profile, log them in
        // Update last login
...
if ($action === 'complete_profile') {
    try {
        ...
        $response['message'] = 'An error occurred during registration. Please try again. Error: ' . $e->getMessage();
        echo json_encode($response);
        exit;
    }
}
...
if ($action === 'login') {
    ...
    if ($remember) {
        ...
        $db->query("UPDATE users SET remember_token = ?, remember_token_expires = ? WHERE user_id = ?")
           ->bind(1, $rmTokenHash)->bind(2, $rmExpires)->bind(3, $user['user_id'])->execute();
        setcookie('sc_remember', $user['user_id'] . ':' . $rmToken, [
