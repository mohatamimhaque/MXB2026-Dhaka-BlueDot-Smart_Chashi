...
switch ($action) {
    ...
    case 'update_alert':
        updateAlert($db, $userId);
        break;
    
    case 'resend_alert':
        resendAlert($db, $userId);
        break;
    ...
    case 'get_advisory':
        getAdvisory($db, $userId);
        break;
    
    case 'update_advisory':
        updateAdvisory($db, $userId);
        break;
    
    case 'toggle_advisory':
        toggleAdvisory($db, $userId);
        break;
...
function issueAlert($db, $userId) {
    $alertType = $_POST['alert_type'] ?? $_POST['alertType'] ?? 'system';
    $title = trim($_POST['title'] ?? '');
    $message = trim($_POST['message'] ?? '');
    $priority = $_POST['severity'] ?? $_POST['priority'] ?? 'medium';
    $targetRegion = $_POST['region'] ?? $_POST['targetRegion'] ?? '';
    $targetFarmer = $_POST['target_farmer'] ?? $_POST['targetFarmer'] ?? '';
...
function getAlert($db, $userId) {
    ...
    try {
        ...
        echo json_encode(['success' => false, 'message' => 'Failed to get alert: ' . $e->getMessage()]);
    }
}

function updateAlert($db, $userId) {
    $alertId = $_POST['alert_id'] ?? '';
    $title = trim($_POST['title'] ?? '');
    $message = trim($_POST['message'] ?? '');
    $alertType = $_POST['alert_type'] ?? 'general';
    ...
    // Get the original alert to find its title (for bulk update)
    $alert = $db->single("SELECT * FROM alerts WHERE alert_id = ? AND created_by = ?", [$alertId, $userId]);
    if (!$alert) {
    ...
    try {
        // Update all alerts with the same title (bulk update for grouped advisories)
        $db->query("UPDATE alerts SET title = ?, message = ?, alert_type = ?, priority = ? WHERE title = ? AND created_by = ?")
           ->bind(1, $title)
           ->bind(2, $message)
           ->bind(3, $alertType)
           ->bind(4, $priority)
           ->bind(5, $originalTitle)
           ->bind(6, $userId)
           ->execute();
        
        echo json_encode(['success' => true, 'message' => 'All matching alerts updated successfully']);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => 'Failed to update alert: ' . $e->getMessage()]);
    }
}

function resendAlert($db, $userId) {
    $alertId = $_POST['alert_id'] ?? '';
    ...
    try {
        // Mark existing as unread and update timestamp
        $db->query("UPDATE alerts SET is_read = 0, created_at = NOW() WHERE alert_id = ?")
           ->bind(1, $alertId)
           ->execute();
        
        echo json_encode(['success' => true, 'message' => 'Alert resent successfully']);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => 'Failed to resend alert: ' . $e->getMessage()]);
    }
}
...
function getAdvisory($db, $userId) {
    ...
    try {
        ...
        echo json_encode(['success' => false, 'message' => 'Failed to get advisory: ' . $e->getMessage()]);
    }
}

function updateAdvisory($db, $userId) {
    ...
    try {
        $db->query("UPDATE advisories SET title = ?, content = ?, advisory_type = ?, priority = ? WHERE advisory_id = ?")
           ->bind(1, $title)
           ->bind(2, $content)
           ->bind(3, $advisoryType)
           ->bind(4, $priority)
        ...
        echo json_encode(['success' => true, 'message' => 'Advisory updated successfully']);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => 'Failed to update advisory: ' . $e->getMessage()]);
    }
}

function toggleAdvisory($db, $userId) {
    $advisoryId = $_POST['advisoryId'] ?? '';
    ...
    try {
        $db->query("UPDATE advisories SET is_active = ? WHERE advisory_id = ?")
           ->bind(1, $isActive)
           ->bind(2, $advisoryId)
           ->execute();
        
        $status = $isActive ? 'activated' : 'deactivated';
        echo json_encode(['success' => true, 'message' => "Advisory $status successfully"]);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => 'Failed to update advisory: ' . $e->getMessage()]);
    }
}
...
function deleteAlert($db, $userId) {
    $alertId = $_POST['alertId'] ?? $_POST['alert_id'] ?? '';
    ...
    // Get the alert to find its title (for bulk delete)
    $alert = $db->single("SELECT * FROM alerts WHERE alert_id = ? AND created_by = ?", [$alertId, $userId]);
    if (!$alert) {
        echo json_encode(['success' => false, 'message' => 'Alert not found or unauthorized']);
        return;
    ...
    try {
        // Delete all alerts with the same title (bulk delete for grouped advisories)
        $db->query("DELETE FROM alerts WHERE title = ? AND created_by = ?")
           ->bind(1, $alertTitle)
           ->bind(2, $userId)
           ->execute();
        echo json_encode(['success' => true, 'message' => 'All matching alerts deleted successfully']);
    } catch (Exception $e) {
