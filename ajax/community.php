...
function addComment($db, $userId) {
    ...
    try {
        ...
        if ($post['user_id'] != $userId) {
            // Check if the post owner still exists in the users table
            $userExists = $db->single("SELECT user_id FROM users WHERE user_id = ?", [$post['user_id']]);
            if ($userExists) {
                try {
                    ...
                    // Silently fail if alert creation fails - comment is still added
                    error_log("Failed to create alert: " . $alertException->getMessage());
                }
            }
        }
...
function getNearbyFarmers($db, $userId) {
    ...
    $userRegion = '';
    $userDistrict = '';
    $profileQuery = "SELECT region, district FROM farmer_profiles WHERE user_id = " . intval($userId);
    ...
    $sql = "SELECT u.user_id, u.first_name, u.last_name, u.phone, u.profile_img_url, u.email,
            fp.region, fp.district, fp.primary_crops, fp.farm_size, fp.experience_level,
            0 as distance
            FROM users u 
            LEFT JOIN farmer_profiles fp ON u.user_id = fp.user_id
            WHERE u.role = 'farmer' AND u.user_id != " . intval($userId);
    
    // Apply search filter
    if (!empty($search)) {
        $searchEsc = mysqli_real_escape_string($conn, $search);
        $sql .= " AND (
            u.first_name LIKE '%{$searchEsc}%' OR 
            u.last_name LIKE '%{$searchEsc}%' OR 
            CONCAT(u.first_name, ' ', u.last_name) LIKE '%{$searchEsc}%' OR
            fp.region LIKE '%{$searchEsc}%' OR 
            fp.district LIKE '%{$searchEsc}%' OR 
            fp.primary_crops LIKE '%{$searchEsc}%'
        )";
    }
    
    // Apply crop filter
    if (!empty($crop)) {
        $cropEsc = mysqli_real_escape_string($conn, $crop);
        $sql .= " AND fp.primary_crops LIKE '%{$cropEsc}%'";
    }
    
    // Apply experience filter
    if (!empty($experience)) {
        $expEsc = mysqli_real_escape_string($conn, $experience);
        $sql .= " AND fp.experience_level = '{$expEsc}'";
    }
    
    // Order by region match first
    $regionEsc = mysqli_real_escape_string($conn, $userRegion);
    $districtEsc = mysqli_real_escape_string($conn, $userDistrict);
    $sql .= " ORDER BY 
              CASE WHEN fp.region = '{$regionEsc}' THEN 0 
                   WHEN fp.district = '{$districtEsc}' THEN 1 
                   ELSE 2 END,
              u.first_name ASC";
    ...
    // Get total count
    $countQuery = "SELECT COUNT(*) as total FROM users WHERE role = 'farmer' AND user_id != " . intval($userId);
    $countResult = mysqli_query($conn, $countQuery);
    $totalFarmers = 0;
    if ($countResult && $row = mysqli_fetch_assoc($countResult)) {
        $totalFarmers = intval($row['total']);
    }
...
function getOfficers($db) {
    ...
    try {
        $sql = "SELECT u.user_id, u.first_name, u.last_name, u.phone, u.email, u.profile_img_url,
               op.region, op.district, op.expertise_area, op.department, op.designation, op.office_location
               FROM users u 
        ...
        if (!empty($search)) {
            $sql .= " AND (u.first_name LIKE ? OR u.last_name LIKE ? OR op.region LIKE ? OR op.district LIKE ? OR op.expertise_area LIKE ? OR op.department LIKE ?)";
            $searchTerm = "%$search%";
            $params[] = $searchTerm;
            $params[] = $searchTerm;
            $params[] = $searchTerm;
        ...
        if (!empty($expertise)) {
            $sql .= " AND op.expertise_area LIKE ?";
            $params[] = "%$expertise%";
        }
        
        $sql .= " ORDER BY op.region ASC, u.first_name ASC LIMIT 50";
        
        $officers = $db->resultSet($sql, $params);
        
        // Get available regions for filter
        $regions = $db->resultSet("SELECT DISTINCT region FROM officer_profiles WHERE region IS NOT NULL AND region != '' ORDER BY region", []);
